...
The compact GIS toolbar provides Pan, Point, Distance, Area, Landmarks, Layers, GPS, Terrain and Display controls. **Locate me**, **North up** and **Maps** are available in the map corner. The default status readout follows the cursor (or map center on touch devices) and shows WGS84, suggested DSM/Kalianpur references, and terrain height. Street, satellite, topographic, custom and offline maps are available. Automatic cursor elevation fetches cached or online DEM tiles; turn it off in Terrain for offline-only use.

All grid-source converters and batch conversion offer **Auto (source candidates)**. Multiple compatible zones are listed rather than guessed; select the map-sheet zone to resolve them. Existing explicit zone defaults and every original identifier are retained. Map entry also supports DSM/Kalianpur Auto candidates. GPS position, marking and tracks reject poor-accuracy fixes and implausible jumps; reported height and uncertainty remain unmodified. These filters do not create RTK precision.
...
The active Map page now uses a compact provider dropdown and Mark / Measure / Directions controls. The previous Map tools panel, Field tools menu, numbered 1–5 toolbar and GIS workbench are removed from the active view. Switch between India topo, MapLibre vector streets, Leaflet/OpenStreetMap and token-enabled Mapbox maps. Add labeled symbols, hide/show labels, measure ellipsoidal distance and true bearings between A/B, or request a separate driving route. [Current map instructions and host configuration](docs/map-workspace.md#compact-map-providers-markers-measurements-and-directions) supersede the earlier advanced-workspace UI description. Existing calculator/grid definitions remain unchanged.
